...
awaitable<void> co_do_sleep(int seconds) noexcept
{
  co_await sleep_for(std::chrono::seconds(seconds));
  std::puts("co_await sleep_for");
}
void do_sleep(int seconds) noexcept
{
...
awaitable<void> test() noexcept {
 execution_context& ctx = co_await this_coro::context;
	for(unsigned int i {}; i < 10; i ++) {
		co_await when_any(co_do_sleep(1), co_do_sleep(3), co_bind(ctx, do_sleep, 2), co_bind(ctx, do_sleep, 4));
		std::puts("----------when_any done----------");
		co_await when_all(co_do_sleep(1), co_do_sleep(3), co_bind(ctx, do_sleep, 2), co_bind(ctx, do_sleep, 4));
